...
#if CONFIG_USE_LCD_240X240_YELLOW || CONFIG_USE_LCD_240X240_COLORFUL
...
private:
    ...
    void InitializeButtons() {
        boot_button_.OnClick([this]() {
            ...
            if (GetNetworkType() == NetworkType::WIFI) {
                if (app.GetDeviceState() == kDeviceStateStarting && !wifi.IsConnected()) {
                    // cast to WifiBoard
    ...
    void InitializeGc9107Display() {
...
#if CONFIG_USE_LCD_240X240_GIF || CONFIG_USE_LCD_240X240_PNG
        // 初始化液晶屏驱动芯片
        ...
        display_ = new MirrorDisplay(panel_io, panel, DISPLAY_WIDTH, DISPLAY_HEIGHT,
...
public:
    ...
    void InitializeTools() {
        ...
        mcp_server.AddTool(
            ...
            "此工具用于获取头部数据检测头部是否被触摸或者抚摸，在与用户互动时经常时刻或每两句对"
            "话都使用此工具\n"
            ...
            PropertyList(), [this](const PropertyList& properties) -> ReturnValue {
                ...
                if (this->Gethead_value(head_value)) {
                    return static_cast<int>(head_value);
                }
                return false;
            });
        mcp_server.AddTool(
            "self.get_body_status",
            "此工具用于获取身体部位数据检测身体部位是否被触摸或者抚摸，在与用户互动时经常时刻或"
            "每两句对话都使用此工具\n"
            ...
            PropertyList(), [this](const PropertyList& properties) -> ReturnValue {
                ...
                if (this->Getbody_value(body_value)) {
